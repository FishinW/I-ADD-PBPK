...
source (file = 'I_ADD_PBPK_Model_2026May22_Brain Three Layer Kidney One Layer_rat_without I-ADD_with fuorgan.R')  # Use the new version of I-ADD model
...
# ----------------------------
data1 <- read.csv("../../Database/PK_Data/Dex/Dex_PK_Dataset_Rat_Unit_Consist_Round4.csv", check.names = FALSE, strip.white = TRUE) %>% 
  filter(Route == "IV") %>% 
  filter(Compartments %in% c("Plasma", "Liver", "Kidney", "Brain")) 

data2 <- read.csv("../../Database/PK_Data/03_Data searching for DEX/Rat from Jericho/Dex_PK_Dataset2_Rat_IV_Unit_Consist_Round4.csv", check.names = FALSE, strip.white = TRUE) %>% 
  filter(Route == "IV") %>% 
...
theta.int <- log(c(
                   ...
                   PR          = 0.13,
                   # PBR         = 1.86,
                   PABRC_VAS   = 0.15,# initial 0.6
                   PABRC_INT   = 0.30, # initial 0.6
                   fu          = 0.175,
                   fucel_brain = 0.24,
                   fuint_brain = 0.29,   
                   BP          = 0.725,
                   CLint       = 0.02     # initial 0.4
))
