...
p_release_60_kidney <- ggplot(release_60_day_kidney, aes(Time, Propotion)) +
...
p_release_60_brain <- ggplot(release_60_day_brain, aes(Time, Propotion)) +
  geom_line(
...
# ---- Save fitted parameters and figures ----
release_first_order_parameters <- bind_rows(
  data.frame(Profile = "30-day brain",  Parameter = names(coef_30_brain),  Estimate = unname(coef_30_brain)),
  data.frame(Profile = "60-day brain",  Parameter = names(coef_60_brain),  Estimate = unname(coef_60_brain)),
  data.frame(Profile = "60-day kidney", Parameter = names(coef_60_kidney), Estimate = unname(coef_60_kidney))
)

write.csv(
  release_first_order_parameters,
  file.path("results", "tables", "release_first_order_fitted_parameters.csv"),
  row.names = FALSE
)

ggsave(file.path("results", "figures", "release_first_order_60day_kidney.png"),
       p_release_60_kidney, width = 8.5, height = 6.5, units = "in", dpi = 600, bg = "white")
ggsave(file.path("results", "figures", "release_first_order_60day_brain.png"),
       p_release_60_brain, width = 8.5, height = 6.5, units = "in", dpi = 600, bg = "white")

print(p_release_60_kidney)
print(p_release_60_brain)
