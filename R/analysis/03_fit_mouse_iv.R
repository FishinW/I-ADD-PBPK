# =============================================================================
# File: 03_fit_mouse_iv.R
# Purpose: Fit and evaluate the mouse IV PBPK model.
# Inputs: Mouse IV PK datasets; mouse PBPK model.
...
source(file.path('R', 'models', '01_model_mouse_iv.R'))  
...
# -------------------------------------------------------------------
# PLOT 2: Obs vs Pred by Study (faceted by organ) (initial model)
# -------------------------------------------------------------------
...
write.csv(data.frame(Parameter = names(pars_log_fit), Estimate = exp(pars_log_fit)),
          file.path("results", "tables", "mouse_iv_fitted_parameters.csv"), row.names = FALSE)
write.csv(R2_by_organ, file.path("results", "tables", "mouse_iv_R2_by_organ.csv"), row.names = FALSE)
write.csv(data.frame(R2_overall_natural = R2_overall_nat,
                     R2_overall_log10 = R2_overall_log),
          file.path("results", "tables", "mouse_iv_R2_overall.csv"), row.names = FALSE)
write.csv(combined, file.path("results", "tables", "mouse_iv_observed_predicted.csv"), row.names = FALSE)

ggsave(file.path("results", "figures", "mouse_iv_goodness_of_fit.png"),
       p_gof, width = 7.5, height = 6.5, units = "in", dpi = 600, bg = "white")

n_mouse_rows <- max(1, dplyr::n_distinct(obs_all$Row_Label))
ggsave(file.path("results", "figures", "mouse_iv_concentration_time_by_study.png"),
       p_all, width = 12, height = max(7, 1.8 * n_mouse_rows),
