# =============================================================================
# File: 04_fit_rat_iv.R
# Purpose: Fit and evaluate the rat IV PBPK model.
# Inputs: Rat IV PK datasets; rat PBPK model.
# Outputs: Model-fitting objects, diagnostic plots, and evaluation metrics.
...
source(file.path('R', 'models', '02_model_rat_iv.R'))  
...
# ----------------------------
data1 <- read.csv("data/pk/Dex_PK_Dataset_Rat_Unit_Consist_Round4.csv", check.names = FALSE, strip.white = TRUE) %>% 
  filter(Route == "IV") %>% 
  filter(Compartments %in% c("Plasma", "Liver", "Kidney", "Brain")) 

data2 <- read.csv("data/pk/Dex_PK_Dataset2_Rat_IV_Unit_Consist_Round4.csv", check.names = FALSE, strip.white = TRUE) %>% 
  filter(Route == "IV") %>% 
...
data <- rbind(data1, data2) %>% 
  ...
  mutate(Number = dense_rank( 
    interaction(F_Author, Study_ID, drop = TRUE))) %>%   
...
top_params <- head(par_names, 10)
top_params

# Subset starting vector
theta_sel <- theta.int[c("CLint")]
...
# -----------------------------------------------------
# Match predictions to observations (fitted model)
# -----------------------------------------------------
data <- data_comb
...
organ_order <- c( "Plasma", "Brain")

year_order <- c(2024, 2018, 2010, 2005, 2003)
...
# Apply factor order to both datasets
obs_all <- obs_all %>%
  mutate(
...
# =============================================================================
# Save model-fitting outputs
# =============================================================================
write.csv(data.frame(Parameter = names(pars_log_fit), Estimate = exp(pars_log_fit)),
          file.path("results", "tables", "rat_iv_fitted_parameters.csv"), row.names = FALSE)
write.csv(R2_by_organ, file.path("results", "tables", "rat_iv_R2_by_organ.csv"), row.names = FALSE)
write.csv(data.frame(R2_overall_natural = R2_overall_nat,
                     R2_overall_log10 = R2_overall_log),
          file.path("results", "tables", "rat_iv_R2_overall.csv"), row.names = FALSE)
write.csv(combined, file.path("results", "tables", "rat_iv_observed_predicted.csv"), row.names = FALSE)

ggsave(file.path("results", "figures", "rat_iv_goodness_of_fit.png"),
       p_gof, width = 7.5, height = 6.5, units = "in", dpi = 600, bg = "white")

n_rat_rows <- max(1, dplyr::n_distinct(obs_all$Row_Label))
ggsave(file.path("results", "figures", "rat_iv_concentration_time_by_study.png"),
       p_all, width = 12, height = max(7, 1.8 * n_rat_rows),
