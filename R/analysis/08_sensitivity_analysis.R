# =============================================================================
# File: 08_sensitivity_analysis.R
# Purpose: Calculate local normalized sensitivity coefficients and circular plots.
# Inputs: AUC-enabled brain and kidney I-ADD models.
# Outputs: Sensitivity tables and circular sensitivity figures.
...
# model files
brain_model_file  <- file.path("R", "models", "06_model_rat_brain_iadd_auc.R")
kidney_model_file <- file.path("R", "models", "07_model_rat_kidney_iadd_auc.R")
...
# =============================================================================
write.csv(brain_nsc, file.path("results", "tables", "brain_AUC_normalized_sensitivity.csv"), row.names = FALSE)
write.csv(kidney_nsc, file.path("results", "tables", "kidney_AUC_normalized_sensitivity.csv"), row.names = FALSE)
if (!is.null(brain_plasma_nsc)) {
  write.csv(brain_plasma_nsc, file.path("results", "tables", "brain_model_plasma_AUC_normalized_sensitivity.csv"), row.names = FALSE)
}
if (!is.null(kidney_plasma_nsc)) {
  write.csv(kidney_plasma_nsc, file.path("results", "tables", "kidney_model_plasma_AUC_normalized_sensitivity.csv"), row.names = FALSE)
}

ggsave(file.path("results", "figures", "brain_AUC_sensitivity_barplot.png"),
       p_brain, width = 8, height = 6.5, units = "in", dpi = 600, bg = "white")
ggsave(file.path("results", "figures", "kidney_AUC_sensitivity_barplot.png"),
       p_kidney, width = 8, height = 6.5, units = "in", dpi = 600, bg = "white")
ggsave(file.path("results", "figures", "AUC_sensitivity_circular.png"),
       p_circle, width = 12, height = 10, units = "in", dpi = 600,
       limitsize = FALSE, bg = "white")
ggsave(file.path("results", "figures", "brain_kidney_AUC_sensitivity_circular.png"),
       p_circle_BK, width = 11, height = 9, units = "in", dpi = 600,
       limitsize = FALSE, bg = "white")
ggsave(file.path("results", "figures", "AUC_sensitivity_top10_circular.png"),
       p_circle_top10, width = 12, height = 10, units = "in", dpi = 600,
       limitsize = FALSE, bg = "white")
