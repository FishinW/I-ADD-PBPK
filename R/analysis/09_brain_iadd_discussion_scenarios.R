# =============================================================================
# File: 09_brain_iadd_discussion_scenarios.R
# Purpose: Run alternative brain I-ADD Monte Carlo scenarios used in the discussion.
# Inputs: 30-day and 60-day brain I-ADD models.
# Outputs: Scenario-specific Monte Carlo summaries and plots.
...
# Parameter table - 30-Day (PABRC_VAS = 15)
mc_params_brain30 <- list(
  normal = data.frame(
    name = c("BW",  "QCC", "QBRC"),
    mean = c(0.25,  14,    0.02),
    CV   = c(0.2,   0.2,   0.2)
  ...
  lognormal = data.frame(
    ...
    mean = c(0.134, 1.29,  11.0,        0.375,       1.0,
             0.24,       0.24,         0.175,  0.725,  0.040),
...
generate_mc_pars_brain <- function(N = 1000, params) {
...
# Generate separate populations for 30-day and 60-day
mc_pars_brain30 <- generate_mc_pars_brain(N = 1000, params = mc_params_brain30)

cat("30-Day MC parameter summary (mean | CV):\n")
print(mc_pars_brain30 %>% select(-MC_ID) %>%
...
# Run MC - 30-Day
cat("\nRunning Monte Carlo for 30-Day Brain I-ADD model (N=1000, PABRC_VAS=15)...\n")

sim_mc_brain30 <- map_dfr(
  split(mc_pars_brain30, mc_pars_brain30$MC_ID),
  run_one_mc_brain,
  model = mod_30Days,
  end   = 1000,
  delta = 0.5
...
# Parameter table - 60-Day (PABRC_VAS = 1.5)
mc_params_brain60 <- list(
  normal = data.frame(
    name = c("BW",  "QCC", "QBRC"),
    mean = c(0.25,  14,    0.02),
    CV   = c(0.2,   0.2,   0.2)
  ...
  lognormal = data.frame(
    name = c("PR",  "PBR", "PABRC_VAS", "PABRC_INT", "PABRC_OUT",
             "PABRC_IN", "fucel_brain", "fu",   "BP",   "CLint"),
    mean = c(0.134, 1.29,  1.1,         0.375,       1.0,
...
# Generate separate population for 60-day
mc_pars_brain60 <- generate_mc_pars_brain(N = 1000, params = mc_params_brain60)

cat("60-Day MC parameter summary (mean | CV):\n")
print(mc_pars_brain60 %>% select(-MC_ID) %>%
        summarise(across(everything(), list(mean = mean, cv = ~sd(.) / mean(.)))))

# Run MC - 60-Day
cat("\nRunning Monte Carlo for 60-Day Brain I-ADD model (N=1000, PABRC_VAS=1.5)...\n")

sim_mc_brain60 <- map_dfr(
  split(mc_pars_brain60, mc_pars_brain60$MC_ID),
  run_one_mc_brain,
  model = mod_60Days,
  end   = 1000,
  delta = 0.5
...
# =============================================================================
# Save discussion-scenario outputs
# =============================================================================
if (exists("mc_pars_brain30")) write.csv(mc_pars_brain30, file.path("results", "tables", "discussion_brain30_mc_parameters.csv"), row.names = FALSE)
if (exists("mc_pars_brain60")) write.csv(mc_pars_brain60, file.path("results", "tables", "discussion_brain60_mc_parameters.csv"), row.names = FALSE)
ggsave(file.path("results", "figures", "discussion_brain_iadd_30day_monte_carlo.png"),
       p_mc_brain30, width = 10.5, height = 6.5, units = "in", dpi = 600, bg = "white")
ggsave(file.path("results", "figures", "discussion_brain_iadd_60day_monte_carlo.png"),
