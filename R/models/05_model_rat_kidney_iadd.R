# =============================================================================
# File: 05_model_rat_kidney_iadd.R
# Purpose: mrgsolve model definition for 05 model rat kidney iadd.
...
PABRC_VAS   : 0.6   : -, Brain vascular to interstitial permeability coefficient. (PABR_L = PABRC_L*QBR)
PABRC_INT   : 0.6   : -, Brain interstitial to vascular permeability coefficient. (PABR_AL = PABRC_AL*QBR)
...
FMAX_REL   : 0.978     : Max release propotion of 60-days release profile
KREL       : 0.0126    : 1/h first-order release rate from device 
...
double PABR_OUT  = QBR;                            // Brain intracellular to interstitial permeability (L/h) starts as “how much blood can leave/enter per time” for each organ
double PABR_IN   = 0.24*QBR;                      // Brain interstitial to intracellular  permeability (L/h) 
...
double RAK = QK*(CA - (CK*BP/PK)) + KREL * IADD;                      // kidney
double RAR = QR*(CA - (CR*BP/PR));                       // rest


// Instantaneous rate of release (amount/hour)
double RATE_IADD = KREL * IADD;   // mg/h
